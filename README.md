# ⚛️ Documentation du Projet (React + Vite)

Ce document regroupe toutes les commandes pour installer, lancer et travailler sur le projet en équipe.

## 🚀 1. Installation (Pour un nouveau développeur)
```bash
# 1. Clone le projet (récupère le dossier)
git clone https://github.com/MakineIUT/sushi.git

# 2. Rentre dans le dossier
cd SushiFast

# 3. Installe les dépendances 
npm install

# 4. Lancer le projet avec Vite
npm run dev

# 5. Récupère toujours les mises à jour des autres pour éviter les bugs
git pull origin main


### 💾. Sauvegarder et envoyer son travail

```bash
# 1. Ajoute les fichiers modifiés à la liste d'attente
git add .

# 2. Enregistre la version avec un message clair
git commit -m "Explication courte de ce que j'ai fait"

# 3. Envoie sur GitHub
git push origin main

